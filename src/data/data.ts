...
export async function getExperienceListingFilterOptions() {
  const { getItineraries } = await import('@/data/itineraries')
  const { buildItinerarySearchFilterOptions } = await import('@/data/itineraries/search')
  const listings = await getItineraries()
  return buildItinerarySearchFilterOptions(listings)
}
