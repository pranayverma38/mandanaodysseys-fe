...
const FlightCard: FC<FlightCardProps> = ({ className = '', data }) => {
      ...
      <div>
        <div className="flex flex-col md:flex-row">
          <div className="w-24 shrink-0 md:w-20 md:pt-7 lg:w-24">
            <Image
              src={airlines.logo}
              className="w-10 dark:brightness-200 dark:contrast-20"
              alt=""
              sizes="40px"
              width={40}
              height={40}
          ...
          <div className="my-5 flex md:my-0">
            <div className="flex shrink-0 flex-col items-center py-2">
              <span className="block h-6 w-6 rounded-full border border-border"></span>
              <span className="my-1 block grow border-l border-dashed border-border"></span>
              <span className="block h-6 w-6 rounded-full border border-border"></span>
            </div>
            <div className="ms-4 space-y-10 text-sm">
              <div className="flex flex-col space-y-1">
                <span className="text-muted-foreground">Monday, August 12 · 10:00</span>
                <span className="font-medium">Tokyo International Airport (HND)</span>
              </div>
              <div className="flex flex-col space-y-1">
                <span className="text-muted-foreground">Monday, August 16 · 10:00</span>
                <span className="font-medium">Singapore International Airport (SIN)</span>
              </div>
            </div>
          </div>
          <div className="border-l border-border md:mx-6 lg:mx-10"></div>
          <ul className="space-y-1 text-sm text-muted-foreground md:space-y-2">
  ...
  return (
    <Disclosure
      as={'div'}
      className={`relative space-y-6 overflow-hidden rounded-2xl border border-border bg-card p-4 transition-shadow hover:shadow-lg-for-card sm:p-6 ${className}`}
    >
      <DisclosureButton as="div" tabIndex={0} className="relative sm:pe-20">
        ...
        <div className="flex flex-col gap-y-6 sm:gap-y-0 md:flex-row md:items-center">
          ...
          <div className="block space-y-1 lg:hidden">
            <div className="flex font-semibold">
              <div>
                ...
                <span className="mt-0.5 flex items-center text-sm font-normal text-muted-foreground">{departure}</span>
              </div>
              ...
              <div>
                ...
                <span className="mt-0.5 flex items-center text-sm font-normal text-muted-foreground">{arrival}</span>
              </div>
            </div>
          ...
          <div className="hidden min-w-[150px] flex-4 lg:block">
            ...
            <div className="mt-0.5 text-sm font-normal text-muted-foreground">{airlines.name}</div>
          </div>

          {/* TIMME */}
          <div className="hidden flex-4 whitespace-nowrap lg:block">
            <div className="text-lg font-medium">{name}</div>
            <div className="mt-0.5 text-sm font-normal text-muted-foreground">{duration}</div>
          </div>
      ...
      <DisclosurePanel className="rounded-2xl border border-border p-4 md:p-8">
        {renderFlightItem()}
