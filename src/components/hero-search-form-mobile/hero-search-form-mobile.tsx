...
const HeroSearchFormMobile = ({
  ...
  className?: string
  initTab?: ListingType
  /** Single-line trigger label (e.g. hero: "Where to?") */
  compact?: boolean
  showTabs?: boolean
}) => {
  ...
  let activeTabName: ListingType = initTab ?? 'Stays'

  // TODO: Check that the pathname matches each listing type.
  if (!initTab && pathname.startsWith('/experience')) {
  ...
  } else if (!initTab && pathname.startsWith('/car')) {
    activeTabName = 'Cars'
  ...
  } else if (!initTab && pathname.startsWith('/flight')) {
  ...
  } else if (!initTab && pathname.startsWith('/stay')) {
    activeTabName = 'Stays'
