  ...
  return (
    <Headless.Field
      ...
      className={clsx(
        className,
        // Base layout
        'grid grid-cols-[1.25rem_1fr] gap-x-4 sm:grid-cols-[1.5rem_1fr]',
        // Control layout
        '*:data-[slot=control]:col-start-1 *:data-[slot=control]:row-start-1',
        // Label layout
        '*:data-[slot=label]:col-start-2 *:data-[slot=label]:row-start-1',
        // Description layout
        '*:data-[slot=description]:col-start-2 *:data-[slot=description]:row-start-2 *:data-[slot=description]:leading-normal',
        // With description
        'has-data-[slot=description]:**:data-[slot=label]:leading-tight has-data-[slot=description]:**:data-[slot=label]:font-medium'
      )}
    />
  )
}
...
const base = [
  // Basic layout
  'relative isolate flex size-5 items-center justify-center rounded-[0.3125rem] sm:size-5.5',
  ...
  return (
    ...
    >
      <span className={clsx([base, colors[color]])}>
        <svg
          className="size-4 stroke-(--checkbox-check) opacity-0 group-data-checked:opacity-100 sm:size-3"
          viewBox="0 0 14 14"
          fill="none"
        >
          {/* Checkmark icon */}
