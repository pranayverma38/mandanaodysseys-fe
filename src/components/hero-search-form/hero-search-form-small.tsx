...
const HeroSearchFormSmall = ({ className, initTab = 'Stays' }: { className?: string; initTab: ListingType }) => {
  return (
    ...
    >
      <Headless.TabList className="flex h-22 justify-center gap-x-10">
        {heroSearchFormTabsData.map((tab) => {
          return (
            <Headless.Tab
              ...
              as="div"
              className={clsx(
                'group relative -mx-3 flex shrink-0 cursor-pointer items-center justify-center px-3 text-gray-400 hover:text-foreground focus-visible:outline-hidden data-[selected]:text-foreground'
              )}
            >
              <div className="relative">
                <span className="sr-only">{tab.name}</span>
                <HugeiconsIcon icon={tab.icon} size={28} />
                <span className="absolute top-full me-2 mt-1 hidden h-0.5 w-full rounded-full bg-foreground group-data-[selected]:block" />
              </div>
            </Headless.Tab>
          )
        })}
